...
- Next automatic sync time is shown in the browser timezone.
- Settings explain that Zepp orienteering often arrives from Intervals.icu as a hike, so Turistika must be ticked for automatic transfer.
- Imported activities without a stored Livelox id are not retried (manual upload / cleared queue).
- Admin hides empty sync runs and shows each user’s last import/skip/error instead of the last idle poll.
- Admin sync-run counts show a hover popup with the user e-mail, activity, and skip/error reason.
- Admin can dismiss the recent-errors list; logs stay, so retries and the queue are unchanged.
